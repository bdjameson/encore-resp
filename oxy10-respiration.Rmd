...
metadata <- metadata %>% 
  ...
  dplyr::select(Species, colony_initial, colony_id, Surface_area, Run_total, Treatment, Chamber.vol.L, Date, Start.time, Stop.time, Light_dark)
...
pr <- left_join(
  ...
  distinct(metadata, Species, colony_initial, colony_id, Surface_area, Run_total, Treatment, Chamber.vol.L))
```
...
```{r}
resp.colony.track <- data %>% subset(Light_dark == 0) %>% 
  ggplot(aes(timepoint, micromol.cm2.h)) +
  #geom_boxplot(aes(color=Species)) +
  geom_point(aes(color=Species, alpha=Treatment_overall)) +
  geom_line(aes(color=Species, alpha=Treatment_overall, shape=Treatment_overall), stat="identity") +
...
ggsave(resp.colony.track, filename = 'output/resp.colony.track.pdf', height = 6, width = 8)
```
